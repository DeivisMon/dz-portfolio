...
export default function IndexComponent() {
  ...
  useEffect(() => {
    ...
    console.log(config.SNAP_LERP_FACTOR);
    ...
    function snapToNearestSlide(basisX = state.targetX) {
      ...
      const currentOffset = basisX + centerOffset;
    ...
    function handleTouchMove(e) {
      ...
      const dx = currentTouchX - (state.lastMoveX ?? currentTouchX);

      if (dt > 0) {
        const instantVelocity = (dx * 1.5) / dt; // px/ms, matches the *1.5 drag multiplier below
        state.touchVelocity =
          (state.touchVelocity || 0) * 0.5 + instantVelocity * 0.5; //smoother changes
      }

      state.lastMoveTime = now;
      state.lastMoveX = currentTouchX;

      const deltaX = (currentTouchX - state.startX) * 1.5;
    ...
    function handleTouchEnd() {
      ...
      if (isVerticalMobile) {
        const MOMENTUM_LOOKAHEAD_MS = 360; // tune: higher = flicks jump further before settling
        const projectedTarget =
          state.targetX + (state.touchVelocity || 0) * MOMENTUM_LOOKAHEAD_MS;
        snapToNearestSlide(projectedTarget);
      }
